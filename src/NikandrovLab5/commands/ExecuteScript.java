...
public class ExecuteScript {
    ...
    public int check(String[] concatenation, Collection collection, String path, FileReader file, Operations operations,
    ...
    public int executeScript(String path, Collection collection, Operations operations, String environmentVariable) {
        try {
            ...
            while (file.ready()) {
                ...
                if (c == '\n') {
                    ...
                    if (!concatenation[0].equals("")) {
                        result = check(concatenation, collection, path, file, operations, environmentVariable);
                    }
                    if (result == 2 | result == 3) {
                        return 2;
                    }
            ...
            int result = 0;
            if (!string.equals("")) {
                result = check(string.toLowerCase().trim().split(" "), collection, path, file, operations, environmentVariable);
            }
            file.close();
            return result;
        } catch (IOException e) {
